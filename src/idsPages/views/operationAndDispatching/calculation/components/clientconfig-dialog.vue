  ...
  <div class="dialog-box">
    ...
    > -->
      <div class="dialog-title">
        <header>
          <div class="panel-parameterName">{{ headTitle }}</div>
          <div class="panel-control">
            <el-button size="small" class="font-size" v-for="(item,index) in btnGroup" type="primary" :key="index" round>{{item.name}}</el-button>
          </div>  
        </header>
      </div>
      <div class="dialog-content">
        <section>
          ...
          >
            ...
            <!-- 计数器固定值 -->
            <el-table-column prop="currentValue" min-width="130">
              <template slot-scope="scope">
                {{
      ...
      <div class="dialog-footer">
        <el-button type="primary" @click="handleClose" class="footer-btn" round>确 定</el-button>
...
import {
  getClientConfigList,
  getClientConfigListEdit,
} from "@/idsPages/api/calculation";
export default {
  data() {
    return {
      dialogVisible: false,
  ...
  props: {
    ...
    paramsList: {//接口所需参数
      type: Object,
      default: {
        // planId: "1461508546909945856",//预案id
        // nodeType:'2',//1场站  2客户
        // nodeCode:'0611_000034',//客户code
        // nodeName:'虎门电厂',//客户名
        // simulationType:'1'//1 24小时仿真 2预案库仿真
      },
    },
  },
  watch: {
    paramsList: {
      handler(newval, oldval) {
        if (newval) {
          let params = {
            planId: this.paramsList.planId,//预案id
            gaugingPointType:'1',//1瞬流 2压力
            stationId:this.paramsList.nodeCode,//门站id
            nodeType: this.paramsList.nodeType //1 场站; 2客户
          }
         getClientConfigList(params).then((res) => {
            console.log(res)
           if (res.resultCode === 0) {
              this.headTitle = res.data.nodeName;
              this.stationInfoList = res.data.list.map((item, index) => {
                let isChildren = true;
              ...
              this.type = this.pageType == 1 ? "hour" : "plan";//1为24小时模式 2为预案裤模式
  ...
  methods: {
    ...
    validateDataBeforeSave(list) {
      for (let i = 0; i < list.length; i++) {
        ...
        // if (this.type == "plan") {//当为预案时 没有选择历史工矿
        //   if (count != 24) {
        //     this.isSave = false;
        //     this.$confirm(
    ...
    submitList(list) {
      ...
      params = {
        list: lists,
        gaugingPointType: '1',
        planId: this.paramsList.planId,
        nodeType: this.paramsList.nodeType,
        nodeCode: this.paramsList.nodeCode,
        nodeName: this.paramsList.nodeName,
        simulationType: this.paramsList.simulationType,
      };
      console.log(params);
      getClientConfigListEdit(params).then((res) => {
        if (res.resultCode === 0) {
          this.$emit('closePopup')
          this.dialogVisible = false;
        }
