<template>
  <div style="width: 100%; height: 100%" class="map-con">
    ...
    <div ref="popupContainer" class="ol-popup">
      <div>
        <ClientconfigDialog v-if="popupType == 'customer'"
                            :pageType="sendChildData.pageType"  
                            :paramsList="sendChildData.paramsList"
                            @closePopup="closePopup"/>
        <StationinfoDialog v-if="popupType == 'station'" 
                           :pageType="sendChildData.pageType"  
                           :paramsList="sendChildData.paramsList"
                           @closePopup="closePopup"/>
      </div>
...
import mapPipe from "@/idsPages/images/operationSituation/map_pipe_count.png";
...
import ClientconfigDialog from "../components/clientconfig-dialog"
import StationinfoDialog from "../components/stationinfo-dialog"
  ...
  methods: {
    ...
    mapEvent() {
      this.map.on("click", (evt) => {
        ...
        let feature = arr[0];
        let hasOverLay = this.overLay.getPosition();console.log(this.overLay.getPosition(),'弹窗---')
