<!--
 * @Description: 输配能力测算页面，包含GIS地图、气量参数获取/配置、仿真计算等功能
 * @author: wanghongxuan
 * @date: 2021-11-09
-->
<template>
  <div class="delivery-wrapper gas-page-bg-diaodu">
    ...
    <div class="content-wrapper content-wrapper-diaodu">
      <!-- 标题 -->
      <div class="title">
        <div class="title-label">输配能力测算</div>
        <div class="title-button">
          <button @click="jumpPlanPage">预案库</button>
        </div>
      </div>
      <!-- 内容部分 -->
          ...
          <div class="computed-header">
            ...
            <div class="computed-start">
              <div
                ...
                @click="clickStartComputed()"
              >
                开始计算
              </div>
              <div v-else class="computed-start-label" @click="finishComputed">
                结束计算
              </div>
...
export default {
  ...
  methods: {
    ...
    finishComputed() {
      const params = {
        simulationCode: "1", // 1代表输配能力测算仿真，2代表预案库仿真
        planId: this.planId,
    ...
    // 预案库页面跳转
    jumpPlanPage() {
      this.$router.push({ path: "/prePlanDatabase" });
    },
    // 气量参数获取/配置弹出框
    setGasConfigDialog() {
    ...
    // 获取planId
    getPlanId() {
      return this.$store.dispatch("calculation/getPlanId");
    },
    // 初始化进入页面时，获取全部门站、全部管网历史记录
    getStationAndPipeHistory() {
      this.loading = true;
      this.showResultText = false;
      // 若finishOrNot！=="1"，则状态为仿真计算进行中
      this.$store.commit("calculation/IS_COMPUTING", true);
