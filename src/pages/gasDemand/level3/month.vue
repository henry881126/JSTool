  ...
  <div>
    <div class="top-card-container">
      <div class="top-card-wrapper top-card-wrapper-1">
        ...
        <div class="date">{{ this.getFormatMonthNext(this.date, '/') }}（下月）</div>
        <div class="data">{{ overviewData.nextMonth }} <span>万方</span></div>
      ...
      <div class="top-card-wrapper top-card-wrapper-2">
        ...
        <div class="date">{{ getFormatMonth(date, '/') }}（本月）</div>
        <div class="data">{{ overviewData.sameMonth }} <span>万方</span></div>
        <div class="label">预测用气量参考</div>
        <svg width="65" height="66" viewBox="0 0 65 66" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="b-wrapper" v-if="descData.length > 0">
      <div class="b-item-wrapper" v-for="(v, i) in descData" :key="i">
        <div class="title">
          ...
          {{ v.name }}
          <el-button size="middle" type="text" class="modify-btn" @click="seeDataDesc(v.name, v.code)">查看</el-button>
        </div>
        <div class="b-item-flex">
          <div class="b-item">
            ...
            <div class="data" v-show="!isModify">{{ v.nextValue }}</div>
            <el-input controls-position="right" size="small" class="custom-input-number" v-show="isModify" v-model="v.nextValue" onkeyup="value=value.replace(/[^\d.]/g,'')"></el-input>
          </div>
          <div class="b-item">
            <div class="label">
            ...
            <div class="data" v-show="!isModify">{{ v.currentValue }}</div>
            <el-input controls-position="right" size="small" class="custom-input-number" v-show="isModify" v-model="v.currentValue" onkeyup="value=value.replace(/[^\d.]/g,'')"></el-input>
          </div>
        </div>
      </div>
      <div class="clear"></div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span :style="{color: scope.row.type === '月环比增长率' || scope.row.type === '月同比增长率' ? '#F56C6C' : '#303133'}">
              {{ scope.row.nextMonth }}
            </span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    getData: function () {
      ...
      api.gasDemand.getDemandMonthOfLevel3(params).then(res => {
        this.overviewData.nextMonth = res[0].nextMonthValue
        this.overviewData.sameMonth = res[0].currentMonthValue
        this.descData = res[0].monthCpmoCompVOList
      })
    },
    seeDataDesc: function (name, code) {
      this.dialogTitle = name
      let params = {
        code: code,
        date: this.getFormatMonth(this.date)
      }
      api.gasDemand.getDemandMonthDescOfLevel3(params).then(res => {
        this.gridData = res[0]
      })
    ...
    saveData: function () {
      ...
      for (let v of this.descData) {
        list.push({
          "code": v.code,
          "currentMonth": parseFloat(v.currentValue),
          "nextMonth": parseFloat(v.nextValue)
        })
      }
      let data = {
        "monthCpmoUpdateDetailVOList": list,
        "date": this.getFormatMonth(this.date)
      }
      api.gasDemand.saveDataMonthOfLevel3(data).then(res => {
        if(res[0]) {
