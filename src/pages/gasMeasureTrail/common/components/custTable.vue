  ...
  <div class="custom-table">
    ...
    >
      ...
      <el-table-column
        ...
        align="left"
        :sort-method="(a, b, type) => sortHandle(a[`${item.prop}`], b[`${item.prop}`], item.prop, type)"
...
export default {
  ...
  methods: { 
     sortHandle (a,b, prop) {
       if (prop === 'forecastDeviationRatio' || prop === 'deviationAssessment') {
         if (Math.abs(Number(a.replace("%","")))>=Math.abs(Number(b.replace("%","")))) {
           return -1;
         }else {
           return 1;
         }
