<template>
  <div class='report-wrapper'>
    ...
    <div class="header">
        ...
        <!-- 提示信息、审批状态 -->
        <div class="tips"><img :src='warningImg' class='tips-time-img' /> 填报时间为当天<span class='tips-time'>10:00</span>之前。</div>
    </div>
    <div class='line'></div>
    <!-- 计划日期 -->
    ...
    <div class="table-wrapper">
      <el-form :model='model' ref='form' :rules='rules'>
        <el-table 
            ...
            ref='dayReportTable'
        >
            <el-table-column  label='序号' prop='id' type='index' :index='1' width='90'></el-table-column>
            <el-table-column  label='资源方' prop='resourcesName' width="250" show-overflow-tooltip></el-table-column>
            <el-table-column  label='月度合同日均量' prop='contractDayAverage' width='170'></el-table-column>
            <el-table-column  label='剩余合同日均量' prop='remainingContractDayAverage' width='170'>
              <template slot-scope="scope">{{ reserveFourDecimal(scope.row.remainingContractDayAverage)}}</template>
            </el-table-column>
            <el-table-column  label='输气站场' prop='downGasPlace' width="150" show-overflow-tooltip></el-table-column>
            ...
            <el-table-column  label='日指定量' prop='dayNum' align='center' width='200'>
              <template slot-scope='scope'>
                <el-form-item :prop="'list.'+scope.$index+'.dayNum'" :rules="rules.dayNum">
                  <el-input
                    v-if='!isDisabled' 
                    size='mini'
                    :disabled='isDisabled' 
                  ...
                  <div v-else>{{ scope.row.dayNum >=0 ? reserveFourDecimal(scope.row.dayNum) : '-' }}</div>
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column  label='批复量' prop='dayNumReply' width="120">
              <template slot-scope="scope">
                <span v-if="checkStatus === '新建' || checkStatus === '审批中'">-</span>
                <span v-else>{{ reserveFourDecimal(scope.row.dayNumReply) }}</span>
              </template>
            </el-table-column>
    ...
    <!-- 底部按钮 -->
    <div class="buttons">
        <el-button :disabled="isDisabled" round class='btn-report' :type="btnType === 'report' ? 'primary' : '' " @click="handleReportAndSave('report')" v-trace="{title:'计划管理-日计划上报-点击确认上报'}" >确认上报</el-button>
...
import { decimalMixin } from '@utils/mixin.js'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        dayNum:[
          {
            required:true,message:'请输入日指定量',trigger:'change'
          },
        ]
      }
    }
  ...
  computed:{
    ...
    calcPlanDate() {
      ...
      let tomorrowTimeStamp = Date.now() + oneDayTimestamp          // 明天
      let afterTomorrowTimeStamp = Date.now() + 2 * oneDayTimestamp     // 后天
    ...
    // 判断时间是否在20点之后，提示文本是填报时间在10点前，但实际情况是在当天20：00之前均可填报
    judegTimeExceed10() {
      return new Date().getHours() >= 20
    },
    isDisabled() {
      return (this.checkStatus === '审批中' || this.checkStatus === '审批通过' ||  this.judegTimeExceed10) ? true :false
    }
  },
  methods: {
    // 统计页面停留时长
    ...
    getDayPlanReportList() {
      ...
      api.planManagement.getDayPlanReportList(params).then(res => {
        ...
        this.model.list.map(item => {
          ...
          item.dayNum = this.reserveFourDecimal(item.dayNum)
        })
    ...
    calcSummary(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if(index === 3) {
            // return sums[index] = this.reserveFourDecimal(parseFloat(this.firstRemainingContractDayAverage))
        ...
        if(sums[5] > 0) {
          sums[5] = this.reserveFourDecimal(sums[5])
        }
        if(sums[5] === 0) {
          sums[5] = ''
        }
    ...
    // 文本框input事件，定义文本框接受数据规则
    handleInputChange(scope) {
      this.$nextTick(()=>{
        console.log('input $index',scope)
        let dayNum = scope.row.dayNum
        dayNum = dayNum.toString()
        if (dayNum.match(/[^\d]/)) {
          dayNum = dayNum.replace(/[^\d.]/g, '') /* 如果输入的是非数字，则清空掉 */
          if(dayNum.indexOf('.') != -1) {      
            dayNum= dayNum.slice(0,dayNum.indexOf('.') + 5)          // 只保留4位小数
          }    
        } else {
            dayNum = dayNum && parseFloat(dayNum)
        }
        scope.row.dayNum = dayNum       // 绑定数据
      })
    },          
  },
  watch: {
  ...
  created() {
    window.zhuge.track('分销中心-计划管理-进入日计划页面',{'页面名称':'用气计划上报页面'})
  },
  mounted() {
    this.getBrowerTime()            // 开始计时，页面停留时间
    // this.getUniqueResource()
    this.getDayPlanReportList()     
  },
  beforeDestroy() {
    clearInterval(this.timer)
    window.zhuge.track('日计划上报页面停留时长',{'日计划上报页面停留时长':this.browerTime})
  }
}
</script>
